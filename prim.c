...
void prim(Graph_t* g) {
	...
	for(int count = 0; count < g->totalVertices-1; count++) {
		...
		while(iter != NULL) {
			if(mstSet[iter->vertex] == 0 && iter->edgeWeight < key[iter->vertex]){
				parent[iter->vertex] = u;
				key[iter->vertex] = iter->edgeWeight;
			}
			iter = iter->next;
		}
	}
